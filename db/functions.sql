...
/****f* functions.sql/getMonitora
 * NAME
 * getMonitora
 * SYNOPSIS
 * Return ID of monitora identified by string
 * ARGUMENTS
 *   * identifier - string - used to identify monitora
 * RETURN VALUE
 * INTEGER - monitora ID 
 ******
 */
CREATE OR REPLACE FUNCTION  getMonitora ( --{{{
  in_identifier VARCHAR(32)
)
RETURNS TABLE (
  monitora_id INTEGER
)
...
BEGIN
  RETURN QUERY
    SELECT m.monitora_id 
      FROM monitoras AS m
     WHERE m.monitora_string = in_identifier;
END;
$FUNC$ LANGUAGE plpgsql SECURITY DEFINER STABLE;
--}}}
...
BEGIN
  RETURN QUERY
    SELECT s.series_id, i.item_id, i.item_string, i.item_number
      FROM series AS s
...
BEGIN
  RETURN QUERY
    INSERT 
      INTO questions AS q
           (item_id, question_string, data_type)
    VALUES (in_item_id, in_question_string, in_type)
...
BEGIN
  RETURN QUERY
    SELECT q.question_id, q.item_id, q.data_type
      FROM questions AS q
 ...
 * ARGUMENTS
 *   * monitora_id - INTEGER - identifier of monitora
...
BEGIN
  RETURN QUERY
    INSERT
      INTO responses AS r
           (monitora_id, week_id)
    VALUES (in_monitora_id, in_week_id)
...
BEGIN
  INSERT
    INTO answers
         (response_id, question_id, numeric_value, string_value)
  VALUES (in_response_id, in_question_id, in_answer_numeric, in_answer_string);
